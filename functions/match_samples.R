noMiss <- function(a_sub) {
    a_sub_nomiss <- a_sub %>% dplyr::select(qpid, sale_id, lon_gd2000_x, lat_gd2000_y, treatment, after_flood, flood, flood_prone, non_flood, flood_analysis_group, one_of(das_vars)) %>% na.omit()

    return(a_sub_nomiss)
}

## Mahalanobis metric with exact matching on dummy variables
## matchMahExactNoRep <- function(a_sub) {
...
matchCEM <- function(a_sub) {
    ...
    ## Match
    m_out <- cem(treatment = "treatment", data = noMiss(a_sub), drop = c('qpid', 'sale_id', 'dist_cbd', 'sale_year', 'lon_gd2000_x', 'lat_gd2000_y', 'ln_sale_price', 'treatment', 'after_flood', 'flood', 'non_flood', 'flood_prone', 'non_flood_prone', 'flood_analysis_group'))
